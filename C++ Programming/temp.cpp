...
{    
   cout<<"   "<<code<<"\t\t"<<note<<"\t\t"<<name<<"\t\t\t"<<tempr<<"\t\t\t"<<tempa<<"\t\t\t"<<price<<"\t\t\t"<<right<<total;                       
   if(availablequantity!=0)
      cout<<"\t"<<availablequantity<<" Not available "<<endl;
   else 
      cout<<endl;
}
  ...
  for(int j=0;j<n2;j++)
  {
    ...
    {  t=item[j].code;
      //cout<<"Item "<<j<<" stock "<<i<<endl;
       if(item[j].code==stoke[i].code && stoke[i].quantity==0)
     {
	stoke[i].note="Not Available";
	stokecount==1;
	stoke[i].total=0;  
	stoke[i].display();   
     }
     else if(item[j].code==stoke[i].code)
      { //cout<<"i "<<i<<endl;
     ...
     if(stokecount==0)
       cout<<"\t_________________________________Item : "<<t<<" not in stoke_________________________________\n";
     if(stokecount==s)
      cout<<"_________________________Not Available in the store_________________________";
     }
     
     
     
  cout<<"\tThe Total Amount : "<<s<<endl;
    cout<<"\nDo want to continue with another purchasing list (1-continue and 0-exit) : ";
     cin>>ch;
  }while(ch!=0);
return 0;
  }
